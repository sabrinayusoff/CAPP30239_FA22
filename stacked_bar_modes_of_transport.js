/* This script creates a stacked bar chart that breaks down the
number of people using various modes of transportation to work
by their earnings.
References: 
- d3 Observable: https://observablehq.com/@d3/stacked-horizontal-bar-chart?collection=@d3/charts
- Source for median earnings in Chicago in 2020: https://data.census.gov/cedsci/table?q=median%20earnings&g=0600000US1703114000&tid=ACSST5Y2020.S2001&moe=false
*/

// earnings = ["$1 to $9,999 or less",
//             "$10,000 to $14,999",
//             "$15,000 to $24,999",
//             "$25,000 to $34,999",
//             "$35,000 to $49,999",
...
d3.csv("data/mode_of_transport_aggregate_combinedearningscategories.csv").then(data => {
    ...
    chart = StackedBarChart(data, {
        ...
        y: d => d.mode,
        z: d => d.earnings,
        xLabel: "No. of Residents →",
        yDomain: d3.groupSort(data, D => d3.sum(D, d => d.count), d => d.mode), // sort y by x
        zDomain: earnings,
        colors: d3.schemeCategory10 // original: d3.schemeSpectral[typesOfDeath.length]
    })

    document.getElementById("mode-of-transport-earnings").appendChild(chart);
    })
